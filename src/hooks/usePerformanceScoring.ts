...
export const usePerformanceScoring = () => {
  ...
  const scorePerformance = useCallback(() => {
    ...
    if (volume > 0) {
      ...
      const volumeVariance = history.volumeReadings.reduce((sum, v) => sum + Math.pow(v - avgVolume, 2), 0) / history.volumeReadings.length;
  ...
  const updateMetricsDisplay = useCallback(() => {
    ...
    setMetrics({
      ...
      volumeConsistency: currentMetrics.volumeConsistency * (1 - smoothingFactor) + current.volumeConsistency * smoothingFactor,
